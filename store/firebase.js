...
export const actions = {
  ...
  async select({ commit }) {
    ...
    const rawResults = await firebase
      .firestore()
      .collection("results")
      .get();
...
export const mutations = {
  renderGraph(_, rawResults) {
    const results = makeResultsToInALine(rawResults)
      .map(v => ({
        ...v,
        t: v.t < 0 ? 0 : v.t
      }))
    ...
    ~chart`scatter--dist-time``scatter`({
      datasets: [
        {
          label: "タッチ誤差と所要時間",
...
//canvas取得
const ctx = idName => id([[idName]]).getContext("2d");

// id取得
const id = id => document.getElementById(id[0]);

// 全ての結果を1つの配列にする
const makeResultsToInALine = results =>
  results.reduce(
    (pre, cur) => [...pre, ...Object.entries(cur).map(v => v[1])],
    []
  );
