...
export const mutations = {
  ...
  resisterResult(state, e) {
    state.results.push({
      ...
      t:
        ~~(
          ((new Date().getTime() + "").slice(5) | 0) -
          (~~(state.previousTime * 1000) / 1000) * 1000
        ) / 1000,
  ...
  setNextState(state) {
    ...
    // 次の座標をランダムに更新
    state.translateX =
      (state.outerWidth / DIVISIONS) * ~~(Math.random() * DIVISIONS);
    state.translateY =
      (state.outerHeight / DIVISIONS) * ~~(Math.random() * DIVISIONS);
